    ...
    if(in_bounds(dim) && ready) {
        if(active) {
            if(just_clicked) {
                ...
                scaled_clicked_position.x = clicked_position.x/CELL_SIZE;
                scaled_clicked_position.y = clicked_position.y/CELL_SIZE;
...
void SelectionRectangular::copy(Canvas& canvas) {
    ...
    for(int i = 0; i < selection.dimensions.x; ++i) {
        for(int j = 0; j < selection.dimensions.y; ++j) {
            clipboard.cell_values.push_back(canvas.tile_val_at(selection.position.x + i, selection.position.y + j, canvas.active_layer));
        }
    }
}
